...
coff = find(dxx<0) ;    % find where envelope returns below threshold
...
if isempty(fl),
   T = round(fs*[-blanking blanking]) ;        % 10/fl was 0.002
else  
   T = round(fs*[-min([10/fl blanking]) blanking]) ;        % 10/fl was 0.002
end
